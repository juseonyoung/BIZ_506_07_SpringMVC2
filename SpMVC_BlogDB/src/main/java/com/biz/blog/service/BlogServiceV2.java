...
public class BlogServiceV2 implements BlogService{
	...
	public int insert(BlogVO blogVO) {
		...
		// date, calendar 라는 클래스를 사용하여 날짜 관련 연산을 수행했으나 많은 이슈들이 있어서 문제를 일으키는 경우가 많았다.
		//java 8에서 새로운 날짜와 시간을 연산하기 위한 클래스가 등장
		
		LocalDate localDate = LocalDate.now();
		LocalTime localTime = LocalTime.now();
		
		// 문자열형 날짜로 변환하기 위해 패턴을 생성
		...
		// insert()를 수행한 후 추가된 데이터 개수만큼을 정수로 return
		// ret가 1이상이면 정상적으로 데이터가 추가됨
