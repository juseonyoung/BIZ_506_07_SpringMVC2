 ...
 * RestTemplate 기능을 사용하여 API 데이터를 가져오기 위한 핵심기능을 사용하기 위한
 * 매우 중요한 model 클래스임
 * API를 통해 가져올 데이터(VO)들을 담을 리스트를 선언하는데 이 때 변수의 이름은 API에서 보내주는
 * 데이터의 이름을 참조하여 만들어야 한다. 
 * 
 */
@Alias("BookList")
@XmlRootElement(name="channel")
public class Book_JSON_Parent { //VO를 감싸는 리스트
	...
	public List<BookVO> items; //네이버가 보내준 데이터의 응답변수 
