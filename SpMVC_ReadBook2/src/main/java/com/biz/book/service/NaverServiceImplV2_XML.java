...
public String queryURL(String category, String bookName) {
		...
		String queryURL =NaverSecret.NAVER_BOOK_XML; //xml방식으로 요청하겟다 jaxb runtime 2.3.3이 필요
	...
	public List<BookVO> getNaverList(String queryURL) {
		...
		ResponseEntity<Book_XML_Parent> bookList = null; //데이터 수신하기 위한 몸체 
		...
		return bookList.getBody().channel.item; // 헤더말고! 바디부분만 추출하여 items 데이터만 추출 실질적인 값만 담김
	}
